package org.asclub.display
{
	...
	public class MovieClipCacher extends EventDispatcher
	{
		...
		//间隔多少帧读取
		private var _intervalFrame:uint = 0;
		...
		public function MovieClipCacher(mc:MovieClip = null,rect:Rectangle = null,start:int = 1,len:int = -1)
		{
			if (mc)
		...
		{
			if (readFrame > endFrame)
			{
				readCompleteHandler();
			}
			else
			{
				if(_intervalFrame == 0 || readFrame % (_intervalFrame + 1) == 1)
				{
