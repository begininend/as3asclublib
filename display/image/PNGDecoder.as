		...
		for(j=0;j<imageInfo[1];j++)
		{
			...
			switch (filter)
			{
				...
				case 2:
				{
					...
					for(i=0;i<index;i++)
					{
//						buffer[movelen+1 +i] += pdat[movelen+1-imageInfo[0]*4-1+i];
		...
		for (j=0;j<imageInfo[1];j++)
		{
			...
			{
//								if (buffer[movelen+1 +(i<<2)+3] != 255)
//				{
//				trace(buffer[movelen+1 +(i<<2)+3]);
//				buffer[movelen+1 +(i<<2)+3] = 255;
//				}
...
/*				var rgb:int = (buffer[movelen+1 +(i<<2)+3]<<24)+(buffer[movelen+1 +(i<<2)+0]<<16)+(buffer[movelen+1 +(i<<2)+1]<<8)+(buffer[movelen+1 +(i<<2)+2])
				bd.setPixel32(i, j, rgb);*/
			}
			movelen += (imageInfo[0]*4+1);
		}
		pdat.position=0;
		return pdat;
/*		bd.setPixels(bd.rect,pdat);
		pdat = bd.getPixels(bd.rect);
		...
		for (j=0;j<imageInfo[1];j++)
		{
			...
			for (i=0;i<imageInfo[0];i++)
			{
				if (pdat[movelen +(i<<2)+0] != 255)
				{
					pdat[movelen +(i<<2)+0] = 255;
				}
		...
		timeNum = getTimer() - timeNum;
//		Alert.show(timeNum.toString());
		return bd; */
		} 
