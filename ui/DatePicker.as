package org.asclub.ui 
{
	...
	public class DatePicker extends Sprite 
	{
		...
		private function onNextClick(e : MouseEvent) : void
		{
			...
			dispatchEvent(new DatePickerEvent(DatePickerEvent.SCROLL));
		}
		...
		//
		private function itemRollOverHandler():void
		{
			
		}
		
		//
		private function itemRollOutHandler():void
		{
		...
		private function drawDateGrid(mm : Number, yy : Number) : void
		{
			...
			{	
				...
				{
					itemBg = new gridItemBg();
				}
				itemBg.x = 3;
				gridItem.addChild(itemBg);
		...
		private function onDateClick(e : MouseEvent) : void
		{
			...
			var currentSeleteDate:Date = new Date(dateEle[2], dateEle[1], dateEle[0]);
			if (!DateUtil.isEqual(currentSeleteDate, _selectedDate, true, false, false))
			{
				this.dispatchEvent(new DatePickerEvent(DatePickerEvent.CHANGE, selectDate));
			}
			selectDate = currentSeleteDate;
			this.dispatchEvent(new DatePickerEvent(DatePickerEvent.ITEM_CLICK, selectDate));
		}
		 ...
		 * 获取一定范围的可选日期。用户不能滚动到可选择范围以外。
		 */
		public function set selectableRange(value:*):void
		{
			if (value is Date)
			{
				_selectableRangeStart = _selectableRangeEnd = value;
			}
			else if (value is Array)
			{
				_selectableRangeStart = value[0];
				_selectableRangeEnd = value[1];
			}
		}
		 

		/**
		 * 获取一定范围的可选日期的开始点。用户不能滚动到可选择范围以外。
		 */
